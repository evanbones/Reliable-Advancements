...
public class JsonEditorWidget extends AbstractWidget {
    ...
    public void renderWidget(@NotNull GuiGraphics gfx, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = startLine; i <= endLine; i++) {
            ...
            gfx.drawString(font, lineNumStr, x + gutterW - numW - 4, lineY + 1, EditorTheme.TEXT_MUTED, false);
    ...
    private void renderLineSelection(GuiGraphics gfx, int lineIdx, int textX, int lineY) {
        int startL, startC, endL, endC;
        if (cursorLine < selectAnchorLine || (cursorLine == selectAnchorLine && cursorCol < selectAnchorCol)) {
            startL = cursorLine;
            startC = cursorCol;
    ...
    private void renderHighlightedLine(GuiGraphics gfx, String line, int textX, int lineY) {
        ...
        while (idx < line.length()) {
            ...
            if (c == '"') {
                ...
                curX = gfx.drawString(font, strToken, curX, lineY + 1, color, false);
                continue;
            }
            ...
            if (Character.isDigit(c) || (c == '-' && idx + 1 < line.length() && Character.isDigit(line.charAt(idx + 1)))) {
                ...
                curX = gfx.drawString(font, numToken, curX, lineY + 1, COLOR_NUMBER, false);
                continue;
            }

            // Boolean / null
            if (line.startsWith("true", idx)) {
                curX = gfx.drawString(font, "true", curX, lineY + 1, COLOR_BOOLEAN, false);
                idx += 4;
                continue;
            }
            if (line.startsWith("false", idx)) {
                curX = gfx.drawString(font, "false", curX, lineY + 1, COLOR_BOOLEAN, false);
                idx += 5;
                continue;
            }
            if (line.startsWith("null", idx)) {
                curX = gfx.drawString(font, "null", curX, lineY + 1, COLOR_BOOLEAN, false);
                idx += 4;
                continue;
            }
            ...
            // Punctuation
            if (c == '{' || c == '}' || c == '[' || c == ']' || c == ':' || c == ',') {
                curX = gfx.drawString(font, String.valueOf(c), curX, lineY + 1, COLOR_PUNCTUATION, false);
                idx++;
                continue;
            }

            // Default fallback
            curX = gfx.drawString(font, String.valueOf(c), curX, lineY + 1, COLOR_DEFAULT, false);
            idx++;
        }
    }
