...
public class EnhancedAdvancementWidget implements IAdvancementEntryGui {
     ...
     */
    public void drawConnection(GuiGraphicsExtractor guiGraphics, EnhancedAdvancementWidget parent, int scrollX, int scrollY, boolean drawInside) {
    ...
    public void draw(GuiGraphicsExtractor guiGraphics, int scrollX, int scrollY, double unzoomedX, double unzoomedY) {
        ...
        if (this.shouldRender()) {
            ...
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, advancementState.frameSprite(this.displayInfo.getType()), scrollX + this.x + 3, scrollY + this.y, ICON_SIZE, ICON_SIZE, finalColor);

            guiGraphics.fakeItem(this.displayInfo.getIcon().create(), scrollX + this.x + 8, scrollY + this.y + 5);

            if (isDimmed) {
                guiGraphics.fill(RenderPipelines.GUI, scrollX + this.x + 8, scrollY + this.y + 5, scrollX + this.x + 24, scrollY + this.y + 21, 0x80000000);
            }
    ...
    public void drawHover(GuiGraphicsExtractor guiGraphics, int scrollX, int scrollY, int left, int top) {
        ...
        if (!this.description.isEmpty()) {
            if (drawTop) {
                guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TITLE_BOX_SPRITE, drawX, drawY + ADVANCEMENT_SIZE - boxHeight, this.width, boxHeight);
            } else {
                guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TITLE_BOX_SPRITE, drawX, drawY, this.width, boxHeight);
            }
        }

        int colorLeft = enhancedDisplayInfo.getTitleColor(stateTitleLeft);
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, stateTitleLeft.boxSprite(), 200, 26, 0, 0, drawX, drawY, j, WIDGET_HEIGHT, colorLeft);
        int colorRight = enhancedDisplayInfo.getTitleColor(stateTitleRight);
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, stateTitleRight.boxSprite(), 200, 26, 200 - k, 0, drawX + j, drawY, k, WIDGET_HEIGHT, colorRight);

        int iconColor = enhancedDisplayInfo.getIconColor(stateIcon);
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, stateIcon.frameSprite(this.displayInfo.getType()), scrollX + this.x + 3, scrollY + this.y, ICON_SIZE, ICON_SIZE, iconColor);

        if (drawLeft) {
            guiGraphics.text(this.minecraft.font, this.title, drawX + 5, scrollY + this.y + 9, -1);

            if (s != null) {
                guiGraphics.text(this.minecraft.font, s, scrollX + this.x - i, scrollY + this.y + 9, -1);
            }
        } else {
            guiGraphics.text(this.minecraft.font, this.title, scrollX + this.x + 32, scrollY + this.y + 9, -1);

            if (s != null) {
                guiGraphics.text(this.minecraft.font, s, scrollX + this.x + this.width - i - 5, scrollY + this.y + 9, -1);
            }
        }
    ...
    public void attachToParent() {
        ...
        this.parents.clear();
        List<Identifier> parentIds = IMultiParentAdvancement.getParents(this.advancementNode.advancement());
