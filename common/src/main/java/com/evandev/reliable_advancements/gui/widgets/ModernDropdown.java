...
public class ModernDropdown extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(@NotNull GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
        ...
        gfx.outline(getX(), getY(), width, height, isOpen ? EditorTheme.BORDER_FOCUSED : (hovered ? EditorTheme.ACCENT_GOLD_MUTED : EditorTheme.BORDER_INNER));
        ...
        gfx.text(font, font.plainSubstrByWidth(label, width - 24), getX() + 6, getY() + (height - 8) / 2, textCol, false);

        EditorTheme.drawChevronIcon(gfx, getX() + width - 16, getY() + (height - 16) / 2, isOpen, hovered);
    }

    public void renderDropdownPopup(GuiGraphicsExtractor gfx, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            gfx.text(font, font.plainSubstrByWidth(opt.label(), dropW - 12), dropX + 6, itemY + 5, textColor, false);
        }
