...
public class ModernEditBox extends EditBox {
    ...
    public void onClick(MouseButtonEvent event, boolean doubleClick) {
        super.onClick(new MouseButtonEvent(event.x() - PAD_X, event.y(), event.buttonInfo()), doubleClick);
    }
    ...
    public void extractWidgetRenderState(@NotNull GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
        ...
        gfx.outline(getX(), getY(), width, height, borderColor);

        int padY = Math.max(0, (height - 9) / 2);
        gfx.pose().pushMatrix();
        gfx.pose().translate(PAD_X, padY);
        super.extractWidgetRenderState(gfx, mouseX - PAD_X, mouseY - padY, partialTick);
        gfx.pose().popMatrix();
    }
}
