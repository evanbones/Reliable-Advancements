...
public class MultiParentHelper {
    ...
    public static List<Identifier> parseParents(JsonObject json) {
        if (json == null) {
            return new ArrayList<>();
        }

        Set<Identifier> parents = new LinkedHashSet<>();
        ...
        if (json.has("parent")) {
            ...
            if (p.isJsonArray()) {
                ...
                Identifier loc = Identifier.tryParse(p.getAsString());
                if (loc != null) {
                    parents.add(loc);
                }
            }
    ...
    public static void applyParentsToJson(JsonObject json, List<Identifier> parents) {
        applyParentsToJson(json, parents, null);
    }

    public static void applyParentsToJson(JsonObject json, List<Identifier> parents, Identifier primaryParent) {
        if (json == null) return;
