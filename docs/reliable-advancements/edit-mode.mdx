...
> **Client-side setting:** `enableEditMode` only affects your own client. Every player who wants to use the editor must enable it themselves. Whether they can actually edit is still gated by the server's permission level check.
...
For condition values, enter valid JSON in the multiline box — a green outline means the JSON is valid, red means there is a syntax error.
...
The advancement tree is updated immediately for all connected players — no reload or restart is needed.

To use these edits in other worlds or distribute them, see [Saving Globally](saving-globally).
